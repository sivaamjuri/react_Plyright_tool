...
| Vite: **`@esbuild/linux-x64` could not be found** (or Rollup `*.linux-x64-gnu.node`) | Optional native deps missing under `master_project/node_modules`. Latest `server.js` runs a one-time `npm install --no-save` for the right `@esbuild/*` and `@rollup/*` package on Linux before starting the dev server; or install manually: `cd backend/master_project && npm install @esbuild/linux-x64 @rollup/rollup-linux-x64-gnu` (on x86_64). |
